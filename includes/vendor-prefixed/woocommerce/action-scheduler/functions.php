 ...
 *
 * @package Justbee_PostCaster_ActionScheduler.
 *
...
function as_enqueue_async_action( $hook, $args = array(), $group = '', $unique = false, $priority = 10 ) {
	if ( ! Justbee_PostCaster_ActionScheduler::is_initialized( __FUNCTION__ ) ) {
		return 0;
	}
	...
	return Justbee_PostCaster_ActionScheduler::factory()->create(
		array(
			'type'      => 'async',
			'hook'      => $hook,
			'arguments' => $args,
...
function as_unschedule_action( $hook, $args = array(), $group = '' ) {
	...
	$action_id = Justbee_PostCaster_ActionScheduler::store()->query_action( $params );

	if ( $action_id ) {
		try {
			Justbee_PostCaster_ActionScheduler::store()->cancel_action( $action_id );
		} catch ( Exception $exception ) {
			Justbee_PostCaster_ActionScheduler::logger()->log(
				$action_id,
				sprintf(
...
function as_unschedule_all_actions( $hook, $args = array(), $group = '' ) {
	...
	if ( empty( $args ) ) {
		if ( ! empty( $hook ) && empty( $group ) ) {
			Justbee_PostCaster_ActionScheduler_Store::instance()->cancel_actions_by_hook( $hook );
			return;
		}
		if ( ! empty( $group ) && empty( $hook ) ) {
			Justbee_PostCaster_ActionScheduler_Store::instance()->cancel_actions_by_group( $group );
			return;
		}
	}
	do {
...
function as_next_scheduled_action( $hook, $args = null, $group = '' ) {
	...
	$params['status'] = Justbee_PostCaster_ActionScheduler_Store::STATUS_RUNNING;
	$action_id        = Justbee_PostCaster_ActionScheduler::store()->query_action( $params );
	if ( $action_id ) {
		return true;
	}

	$params['status'] = Justbee_PostCaster_ActionScheduler_Store::STATUS_PENDING;
	$action_id        = Justbee_PostCaster_ActionScheduler::store()->query_action( $params );
	if ( null === $action_id ) {
		return false;
	}

	$action         = Justbee_PostCaster_ActionScheduler::store()->fetch_action( $action_id );
...
function as_has_scheduled_action( $hook, $args = null, $group = '' ) {
	...
	$query_args = array(
		'hook'    => $hook,
		'status'  => array( Justbee_PostCaster_ActionScheduler_Store::STATUS_RUNNING, Justbee_PostCaster_ActionScheduler_Store::STATUS_PENDING ),
		'group'   => $group,
		'orderby' => 'none',
	);
	...
	$action_id = Justbee_PostCaster_ActionScheduler::store()->query_action( $query_args );
 ...
 *         'status' => '' - Justbee_PostCaster_ActionScheduler_Store::STATUS_COMPLETE or Justbee_PostCaster_ActionScheduler_Store::STATUS_PENDING.
 ...
 *
 * @return Justbee_PostCaster_ActionScheduler_DateTime
 */
function as_get_datetime_object( $date_string = null, $timezone = 'UTC' ) {
	if ( is_object( $date_string ) && $date_string instanceof DateTime ) {
		$date = new Justbee_PostCaster_ActionScheduler_DateTime( $date_string->format( 'Y-m-d H:i:s' ), new DateTimeZone( $timezone ) );
	} elseif ( is_numeric( $date_string ) ) {
		$date = new Justbee_PostCaster_ActionScheduler_DateTime( '@' . $date_string, new DateTimeZone( $timezone ) );
	} else {
		$date = new Justbee_PostCaster_ActionScheduler_DateTime( null === $date_string ? 'now' : $date_string, new DateTimeZone( $timezone ) );
	}
	return $date;
}
