...
use Justbee_PostCaster_ActionScheduler_DataController as ActionScheduler_DataController;
use Justbee_PostCaster_ActionScheduler_LoggerSchema as ActionScheduler_LoggerSchema;
use Justbee_PostCaster_ActionScheduler_StoreSchema as ActionScheduler_StoreSchema;
use Justbee\PostCaster\Vendor\Action_Scheduler\WP_CLI\ProgressBar;
...
class Controller {
	 ...
	 * Scheduler instance.
	 *
	 * @var Justbee\PostCaster\Vendor\Action_Scheduler\Migration\Scheduler
	 */
	...
	public function get_store_class( $class ) {
		if ( \Justbee_PostCaster_ActionScheduler_DataController::is_migration_complete() ) {
			return \Justbee_PostCaster_ActionScheduler_DataController::DATASTORE_CLASS;
		} elseif ( \Justbee_PostCaster_ActionScheduler_Store::DEFAULT_CLASS !== $class ) {
	...
	public function get_logger_class( $class ) {
		...
		if ( $this->has_custom_datastore() ) {
			...
			return \Justbee_PostCaster_ActionScheduler_DataController::LOGGER_CLASS;
		}
	}
	...
	public function get_migration_config_object() {
		...
		if ( ! $config ) {
			$source_store  = $this->store_classname ? new $this->store_classname() : new \Justbee_PostCaster_ActionScheduler_wpPostStore();
			$source_logger = $this->logger_classname ? new $this->logger_classname() : new \Justbee_PostCaster_ActionScheduler_wpCommentLogger();

			$config = new Config();
			$config->set_source_store( $source_store );
			$config->set_source_logger( $source_logger );
			$config->set_destination_store( new \Justbee_PostCaster_ActionScheduler_DBStoreMigrator() );
			$config->set_destination_logger( new \Justbee_PostCaster_ActionScheduler_DBLogger() );
	...
	public function hook_admin_notices() {
		if ( ! $this->allow_migration() || \Justbee_PostCaster_ActionScheduler_DataController::is_migration_complete() ) {
			return;
		}
	...
	public function allow_migration() {
		if ( ! \Justbee_PostCaster_ActionScheduler_DataController::dependencies_met() ) {
			return false;
		}
