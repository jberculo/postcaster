...
/**
 * Class Justbee_PostCaster_ActionScheduler_wpCommentLogger
 *
 * @license GPL-3.0-or-later
 * Modified by justbee on 08-May-2026 using {@see https://github.com/BrianHenryIE/strauss}.
 */
class Justbee_PostCaster_ActionScheduler_wpCommentLogger extends Justbee_PostCaster_ActionScheduler_Logger {
	const AGENT = 'Justbee_PostCaster_ActionScheduler';
	 ...
	 *
	 * @return Justbee_PostCaster_ActionScheduler_LogEntry
	 */
	public function get_entry( $entry_id ) {
		$comment = $this->get_comment( $entry_id );

		if ( empty( $comment ) || self::TYPE !== $comment->comment_type ) {
			return new Justbee_PostCaster_ActionScheduler_NullLogEntry();
		}

		$date = as_get_datetime_object( $comment->comment_date_gmt );
		Justbee_PostCaster_ActionScheduler_TimezoneHelper::set_local_timezone( $date );
		return new Justbee_PostCaster_ActionScheduler_LogEntry( $comment->comment_post_ID, $comment->comment_content, $date );
	}
	 ...
	 * will be regenerated next time Justbee_PostCaster_ActionScheduler_wpCommentLogger::filter_comment_count() is called.
	 */
