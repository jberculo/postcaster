...
/**
 * Class Justbee_PostCaster_ActionScheduler_DBStore
 *
 ...
 */
class Justbee_PostCaster_ActionScheduler_DBStore extends Justbee_PostCaster_ActionScheduler_Store {
	...
	public function init() {
		$table_maker = new Justbee_PostCaster_ActionScheduler_StoreSchema();
		$table_maker->init();
	 ...
	 */
	public function save_unique_action( Justbee_PostCaster_ActionScheduler_Action $action, ?DateTime $scheduled_date = null ) {
	 ...
	 * @throws \RuntimeException     Throws exception when saving the action fails.
	 */
	private function save_action_to_db( Justbee_PostCaster_ActionScheduler_Action $action, ?DateTime $date = null, $unique = false ) {
		global $wpdb;
...
",
	...
	public function fetch_action( $action_id ) {
		...
		foreach ( $date_fields as $date_field ) {
			if ( is_null( $data->$date_field ) ) {
				$data->$date_field = Justbee_PostCaster_ActionScheduler_StoreSchema::DEFAULT_DATE;
			}
		}
	 ...
	 *
	 * @return Justbee_PostCaster_ActionScheduler_Action|Justbee_PostCaster_ActionScheduler_CanceledAction|Justbee_PostCaster_ActionScheduler_FinishedAction
	 */
	protected function make_action_from_db_record( $data ) {
		...
		return Justbee_PostCaster_ActionScheduler::factory()->get_stored_action( $data->status, $data->hook, $args, $schedule, $group, $data->priority );
	}
	...
	public function get_date( $action_id ) {
		...
		Justbee_PostCaster_ActionScheduler_TimezoneHelper::set_local_timezone( $date );
		return $date;
	}
	...
	protected function claim_actions( $claim_id, $limit, ?DateTime $before_date = null, $hooks = array(), $group = '' ) {
		...
		if ( ! empty( $group ) ) {
			...
			// throw exception if no matching group(s) found, this matches Justbee_PostCaster_ActionScheduler_wpPostStore's behaviour.
			if ( empty( $group_ids ) ) {
