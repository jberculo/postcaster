...
/**
 * Class Justbee_PostCaster_ActionScheduler_DBLogger
 *
 ...
 */
class Justbee_PostCaster_ActionScheduler_DBLogger extends Justbee_PostCaster_ActionScheduler_Logger {
	...
	private function create_entry_from_db_record( $record ) {
		...
		if ( is_null( $record->log_date_gmt ) ) {
			$date = as_get_datetime_object( Justbee_PostCaster_ActionScheduler_StoreSchema::DEFAULT_DATE );
		} else {
			$date = as_get_datetime_object( $record->log_date_gmt );
		}

		return new Justbee_PostCaster_ActionScheduler_LogEntry( $record->action_id, $record->message, $date );
	}
	...
	public function init() {
		$table_maker = new Justbee_PostCaster_ActionScheduler_LoggerSchema();
		$table_maker->init();
