...
/**
 * Class Justbee_PostCaster_ActionScheduler_wpPostStore
 *
 * @license GPL-3.0-or-later
 * Modified by justbee on 08-May-2026 using {@see https://github.com/BrianHenryIE/strauss}.
 */
class Justbee_PostCaster_ActionScheduler_wpPostStore extends Justbee_PostCaster_ActionScheduler_Store {
	 ...
	 * Save action.
	 *
	 * @param Justbee_PostCaster_ActionScheduler_Action $action Scheduled Action.
	 ...
	 * @return int
	 */
	public function save_action( Justbee_PostCaster_ActionScheduler_Action $action, ?DateTime $scheduled_date = null ) {
		try {
	 ...
	 */
	protected function create_post_array( Justbee_PostCaster_ActionScheduler_Action $action, ?DateTime $scheduled_date = null ) {
		$post = array(
	...
	public function fetch_action( $action_id ) {
		...
		} catch ( Justbee_PostCaster_ActionScheduler_InvalidActionException $exception ) {
	 ...
	 * Get NULL action.
	 *
	 * @return Justbee_PostCaster_ActionScheduler_NullAction
	 */
	protected function get_null_action() {
		return new Justbee_PostCaster_ActionScheduler_NullAction();
	}
	...
	protected function make_action_from_post( $post ) {
		...
		return Justbee_PostCaster_ActionScheduler::factory()->get_stored_action( $this->get_action_status_by_post_status( $post->post_status ), $hook, $args, $schedule, $group );
	}
	 ...
	 *
	 * @see Justbee_PostCaster_ActionScheduler_Store::query_actions for $query arg usage.
	 *
	 ...
	 *
	 * @param int $action_id Action ID.
	 * @return Justbee_PostCaster_ActionScheduler_DateTime The date the action is schedule to run, or the date that it ran.
	 */
	public function get_date( $action_id ) {
		$next = $this->get_date_gmt( $action_id );
		return Justbee_PostCaster_ActionScheduler_TimezoneHelper::set_local_timezone( $next );
	}
	...
	public function stake_claim( $max_actions = 10, ?DateTime $before_date = null, $hooks = array(), $group = '' ) {
		...
		return new Justbee_PostCaster_ActionScheduler_ActionClaim( $claim_id, $action_ids );
	}
	...
	protected function claim_actions( $claim_id, $limit, ?DateTime $before_date = null, $hooks = array(), $group = '' ) {
		...
		$params[] = Justbee_PostCaster_ActionScheduler_Store::STATUS_PENDING;
	...
	protected function get_actions_by_group( $group, $limit, DateTime $date ) {
		...
		$query_args = array(
			'fields'           => 'ids',
			'post_type'        => self::POST_TYPE,
			'post_status'      => Justbee_PostCaster_ActionScheduler_Store::STATUS_PENDING,
			'has_password'     => false,
	 ...
	 *
	 * @param Justbee_PostCaster_ActionScheduler_ActionClaim $claim Claim object to release.
	 * @return void
	 * @throws RuntimeException When the claim is not unlocked.
	 */
	public function release_claim( Justbee_PostCaster_ActionScheduler_ActionClaim $claim ) {
		/**
		 * Global wpdb object.
		 *
		 * @var wpdb $wpdb
	 ...
	 *
	 * @param Justbee_PostCaster_ActionScheduler_Action $action Action object.
	 */
	protected function validate_action( Justbee_PostCaster_ActionScheduler_Action $action ) {
		try {
			...
			_doing_it_wrong( 'Justbee_PostCaster_ActionScheduler_Action::$args', esc_html( $message ), '2.1.0' );
		}
	}
	...
	public function init() {
		...
		$post_type_registrar = new Justbee_PostCaster_ActionScheduler_wpPostStore_PostTypeRegistrar();
		$post_type_registrar->register();

		$post_status_registrar = new Justbee_PostCaster_ActionScheduler_wpPostStore_PostStatusRegistrar();
		$post_status_registrar->register();

		$taxonomy_registrar = new Justbee_PostCaster_ActionScheduler_wpPostStore_TaxonomyRegistrar();
