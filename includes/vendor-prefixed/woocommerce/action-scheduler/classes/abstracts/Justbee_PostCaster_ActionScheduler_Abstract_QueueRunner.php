 ...
 */
abstract class Justbee_PostCaster_ActionScheduler_Abstract_QueueRunner extends Justbee_PostCaster_ActionScheduler_Abstract_QueueRunner_Deprecated {

	/**
	 * Justbee_PostCaster_ActionScheduler_QueueCleaner instance.
	 *
	 * @var Justbee_PostCaster_ActionScheduler_QueueCleaner
	 */
	protected $cleaner;

	/**
	 * Justbee_PostCaster_ActionScheduler_FatalErrorMonitor instance.
	 *
	 * @var Justbee_PostCaster_ActionScheduler_FatalErrorMonitor
	 */
	protected $monitor;
	...
	/**
	 * Justbee_PostCaster_ActionScheduler_Abstract_QueueRunner constructor.
	 *
	 * @param Justbee_PostCaster_ActionScheduler_Store|null             $store Store object.
	 * @param Justbee_PostCaster_ActionScheduler_FatalErrorMonitor|null $monitor Monitor object.
	 * @param Justbee_PostCaster_ActionScheduler_QueueCleaner|null      $cleaner Cleaner object.
	 */
	public function __construct( ?Justbee_PostCaster_ActionScheduler_Store $store = null, ?Justbee_PostCaster_ActionScheduler_FatalErrorMonitor $monitor = null, ?Justbee_PostCaster_ActionScheduler_QueueCleaner $cleaner = null ) {

		$this->created_time = microtime( true );

		$this->store   = $store ? $store : Justbee_PostCaster_ActionScheduler_Store::instance();
		$this->monitor = $monitor ? $monitor : new Justbee_PostCaster_ActionScheduler_FatalErrorMonitor( $this->store );
		$this->cleaner = $cleaner ? $cleaner : new Justbee_PostCaster_ActionScheduler_QueueCleaner( $this->store );
	}
	...
	public function process_action( $action_id, $context = '' ) {
		...
		try {
			try {
				...
				if ( Justbee_PostCaster_ActionScheduler_Store::STATUS_PENDING !== $this->store->get_status( $action_id ) ) {
					$valid_action = false;
				...
				// compatibility with Justbee_PostCaster_ActionScheduler_Logger.
		...
		if ( isset( $action ) && is_a( $action, 'Justbee_PostCaster_ActionScheduler_Action' ) && $action->get_schedule()->is_recurring() ) {
	 ...
	 * @param int                    $action_id Action ID.
	 */
	protected function schedule_next_instance( Justbee_PostCaster_ActionScheduler_Action $action, $action_id ) {
		// If a recurring action has been consistently failing, we may wish to stop rescheduling it.
		if (
			Justbee_PostCaster_ActionScheduler_Store::STATUS_FAILED === $this->store->get_status( $action_id )
			&& $this->recurring_action_is_consistently_failing( $action, $action_id )
		) {
			Justbee_PostCaster_ActionScheduler_Logger::instance()->log(
				$action_id,
		...
		try {
			Justbee_PostCaster_ActionScheduler::factory()->repeat( $action );
		} catch ( Exception $e ) {
	 ...
	 *
	 * @param Justbee_PostCaster_ActionScheduler_Action $action    The recurring action to be rescheduled.
	 * @param int                    $action_id The ID of the recurring action.
	 *
	 * @return bool
	 */
	private function recurring_action_is_consistently_failing( Justbee_PostCaster_ActionScheduler_Action $action, $action_id ) {
		/**
		...
		$query_args = array(
			...
			'status'       => Justbee_PostCaster_ActionScheduler_Store::STATUS_FAILED,
		 ...
		 * @param Justbee_PostCaster_ActionScheduler_Action $action                  The action being assessed.
		 */
	...
	protected function get_memory_limit() {
		...
		return Justbee_PostCaster_ActionScheduler_Compatibility::convert_hr_to_bytes( $memory_limit );
	}
