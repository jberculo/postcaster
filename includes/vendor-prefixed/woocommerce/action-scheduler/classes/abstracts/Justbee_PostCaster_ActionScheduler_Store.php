...
/**
 * Class Justbee_PostCaster_ActionScheduler_Store
 *
 * @codeCoverageIgnore
 *
 ...
 */
abstract class Justbee_PostCaster_ActionScheduler_Store extends Justbee_PostCaster_ActionScheduler_Store_Deprecated {
	const STATUS_COMPLETE = 'complete';
	const STATUS_PENDING  = 'pending';
	const STATUS_RUNNING  = 'in-progress';
	const STATUS_FAILED   = 'failed';
	const STATUS_CANCELED = 'canceled';
	const DEFAULT_CLASS   = 'Justbee_PostCaster_ActionScheduler_wpPostStore';
	 ...
	 * Save action.
	 *
	 * @param Justbee_PostCaster_ActionScheduler_Action $action Action to save.
	 ...
	 */
	abstract public function save_action( Justbee_PostCaster_ActionScheduler_Action $action, ?DateTime $scheduled_date = null );
	 ...
	 *
	 * @return Justbee_PostCaster_ActionScheduler_Action
	 */
	 ...
	 * @since 3.3.0
	 *
	 * @see Justbee_PostCaster_ActionScheduler_Store::query_actions for $query arg usage but 'per_page' and 'offset' can't be used.
	 *
	 ...
	 *
	 * @return Justbee_PostCaster_ActionScheduler_ActionClaim
	 */
	 ...
	 * Release the claim.
	 *
	 * @param Justbee_PostCaster_ActionScheduler_ActionClaim $claim Claim object.
	 */
	abstract public function release_claim( Justbee_PostCaster_ActionScheduler_ActionClaim $claim );
	 ...
	 *
	 * @param Justbee_PostCaster_ActionScheduler_Action $action Action.
	 * @param null|DateTime          $scheduled_date Action's schedule date (optional).
	 * @return string
	 */
	protected function get_scheduled_date_string( Justbee_PostCaster_ActionScheduler_Action $action, ?DateTime $scheduled_date = null ) {
	 ...
	 * Get the time MySQL formatted date/time string for an action's (next) scheduled date.
	 *
	 * @param Justbee_PostCaster_ActionScheduler_Action|null $action Action.
	 * @param null|DateTime               $scheduled_date Action's scheduled date (optional).
	 * @return string
	 */
	protected function get_scheduled_date_string_local( Justbee_PostCaster_ActionScheduler_Action $action, ?DateTime $scheduled_date = null ) {
		$next = is_null( $scheduled_date ) ? $action->get_schedule()->get_date() : $scheduled_date;
		...
		Justbee_PostCaster_ActionScheduler_TimezoneHelper::set_local_timezone( $next );
		return $next->format( 'Y-m-d H:i:s' );
	}
	 ...
	 *
	 * @throws Justbee_PostCaster_ActionScheduler_InvalidActionException When the decoded arguments are invalid.
	 */
	protected function validate_args( $args, $action_id ) {
		// Ensure we have an array of args.
		if ( ! is_array( $args ) ) {
			throw Justbee_PostCaster_ActionScheduler_InvalidActionException::from_decoding_args( $action_id );
		}

		// Validate JSON decoding if possible.
		if ( function_exists( 'json_last_error' ) && JSON_ERROR_NONE !== json_last_error() ) {
			throw Justbee_PostCaster_ActionScheduler_InvalidActionException::from_decoding_args( $action_id, $args );
		}
	}

	/**
	 * Validate a Justbee_PostCaster_ActionScheduler_Schedule object.
	 *
	 * @param mixed $schedule  The unserialized Justbee_PostCaster_ActionScheduler_Schedule object.
	 * @param int   $action_id The action ID.
	 *
	 * @throws Justbee_PostCaster_ActionScheduler_InvalidActionException When the schedule is invalid.
	 */
	protected function validate_schedule( $schedule, $action_id ) {
		if ( empty( $schedule ) || ! is_a( $schedule, 'Justbee_PostCaster_ActionScheduler_Schedule' ) ) {
			throw Justbee_PostCaster_ActionScheduler_InvalidActionException::from_schedule( $action_id, $schedule );
		}
	}
	 ...
	 *
	 * @param  Justbee_PostCaster_ActionScheduler_Action $action Action to be validated.
	 * @throws InvalidArgumentException When json encoded args is too long.
	 */
	protected function validate_action( Justbee_PostCaster_ActionScheduler_Action $action ) {
		if ( strlen( wp_json_encode( $action->get_args() ) ) > static::$max_args_length ) {
			// translators: %d is a number (maximum length of action arguments).
			throw new InvalidArgumentException( sprintf( __( 'Justbee_PostCaster_ActionScheduler_Action::$args too long. To ensure the args column can be indexed, action args should not be more than %d characters when encoded as JSON.', 'action-scheduler' ), static::$max_args_length ) );
		}
	}
	 ...
	 * Get instance.
	 *
	 * @return Justbee_PostCaster_ActionScheduler_Store
	 */
