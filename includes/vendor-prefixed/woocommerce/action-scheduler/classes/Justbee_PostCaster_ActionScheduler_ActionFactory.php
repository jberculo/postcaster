...
/**
 * Class Justbee_PostCaster_ActionScheduler_ActionFactory
 *
...
class Justbee_PostCaster_ActionScheduler_ActionFactory {
	 ...
	 * @param Justbee_PostCaster_ActionScheduler_Schedule|null $schedule The action's schedule.
	 * @param string                        $group A group to put the action in.
	 * phpcs:ignore Squiz.Commenting.FunctionComment.ExtraParamComment
	 * @param int                           $priority The action priority.
	 *
	 * @return Justbee_PostCaster_ActionScheduler_Action An instance of the stored action.
	 */
	public function get_stored_action( $status, $hook, array $args = array(), ?Justbee_PostCaster_ActionScheduler_Schedule $schedule = null, $group = '' ) {
		...
		switch ( $status ) {
			case Justbee_PostCaster_ActionScheduler_Store::STATUS_PENDING:
				$action_class = 'Justbee_PostCaster_ActionScheduler_Action';
				break;
			case Justbee_PostCaster_ActionScheduler_Store::STATUS_CANCELED:
				$action_class = 'Justbee_PostCaster_ActionScheduler_CanceledAction';
				if ( ! is_null( $schedule ) && ! is_a( $schedule, 'Justbee_PostCaster_ActionScheduler_CanceledSchedule' ) && ! is_a( $schedule, 'Justbee_PostCaster_ActionScheduler_NullSchedule' ) ) {
					$schedule = new Justbee_PostCaster_ActionScheduler_CanceledSchedule( $schedule->get_date() );
				}
				break;
			default:
				$action_class = 'Justbee_PostCaster_ActionScheduler_FinishedAction';
				break;
		}
		 ...
		 *
		 * @param Justbee_PostCaster_ActionScheduler_Action   $action The instantiated action.
		 * @param string                   $hook The instantiated action's hook.
		 * @param array                    $args The instantiated action's args.
		 * @param Justbee_PostCaster_ActionScheduler_Schedule $schedule The instantiated action's schedule.
	...
	public function async_unique( $hook, $args = array(), $group = '', $unique = true ) {
		$schedule = new Justbee_PostCaster_ActionScheduler_NullSchedule();
		$action   = new Justbee_PostCaster_ActionScheduler_Action( $hook, $args, $schedule, $group );
	...
	public function single_unique( $hook, $args = array(), $when = null, $group = '', $unique = true ) {
		...
		$schedule = new Justbee_PostCaster_ActionScheduler_SimpleSchedule( $date );
		$action   = new Justbee_PostCaster_ActionScheduler_Action( $hook, $args, $schedule, $group );
	...
	public function recurring_unique( $hook, $args = array(), $first = null, $interval = null, $group = '', $unique = true ) {
		...
		$schedule = new Justbee_PostCaster_ActionScheduler_IntervalSchedule( $date, $interval );
		$action   = new Justbee_PostCaster_ActionScheduler_Action( $hook, $args, $schedule, $group );
		return $unique ? $this->store_unique_action( $action ) : $this->store( $action );
	}
	...
	public function cron_unique( $hook, $args = array(), $base_timestamp = null, $schedule = null, $group = '', $unique = true ) {
		...
		$cron     = Justbee_PostCaster_CronExpression::factory( $schedule );
		$schedule = new Justbee_PostCaster_ActionScheduler_CronSchedule( $date, $cron );
		$action   = new Justbee_PostCaster_ActionScheduler_Action( $hook, $args, $schedule, $group );
		return $unique ? $this->store_unique_action( $action ) : $this->store( $action );
	}
	 ...
	 *
	 * @param Justbee_PostCaster_ActionScheduler_Action $action The existing action.
	 *
	...
	public function repeat( $action ) {
		...
		$new_action     = new Justbee_PostCaster_ActionScheduler_Action( $action->get_hook(), $action->get_args(), $new_schedule, $action->get_group() );
	...
	public function create( array $options = array() ) {
		...
		switch ( $options['type'] ) {
			...
			case 'cron':
				...
				$cron     = Justbee_PostCaster_CronExpression::factory( $options['pattern'] );
				$schedule = new Justbee_PostCaster_ActionScheduler_CronSchedule( $date, $cron );
				break;

			case 'recurring':
				$date     = as_get_datetime_object( $options['when'] );
				$schedule = new Justbee_PostCaster_ActionScheduler_IntervalSchedule( $date, $options['pattern'] );
				break;
		...
		$action = new Justbee_PostCaster_ActionScheduler_Action( $options['hook'], $options['arguments'], $schedule, $options['group'] );
	 ...
	 *
	 * @param Justbee_PostCaster_ActionScheduler_Action $action Action object to save.
	 *
	 * @return int The ID of the stored action
	 */
	protected function store( Justbee_PostCaster_ActionScheduler_Action $action ) {
		$store = Justbee_PostCaster_ActionScheduler_Store::instance();
	 ...
	 *
	 * @param Justbee_PostCaster_ActionScheduler_Action $action Action object to store.
	 *
	 * @return int ID of the created action. Will be 0 if action was not created.
	 */
	protected function store_unique_action( Justbee_PostCaster_ActionScheduler_Action $action ) {
		$store = Justbee_PostCaster_ActionScheduler_Store::instance();
