 ...
 */
class Justbee_PostCaster_ActionScheduler_ListTable extends Justbee_PostCaster_ActionScheduler_Abstract_ListTable {
	 ...
	 *
	 * @var Justbee_PostCaster_ActionScheduler_Logger
	 */
	protected $logger;

	/**
	 * A Justbee_PostCaster_ActionScheduler_QueueRunner runner instance (or child class)
	 *
	 * @var Justbee_PostCaster_ActionScheduler_QueueRunner
	 */
	protected $runner;
	 ...
	 *
	 * @param Justbee_PostCaster_ActionScheduler_Store       $store Store object.
	 * @param Justbee_PostCaster_ActionScheduler_Logger      $logger Logger object.
	 * @param Justbee_PostCaster_ActionScheduler_QueueRunner $runner Runner object.
	 */
	public function __construct( Justbee_PostCaster_ActionScheduler_Store $store, Justbee_PostCaster_ActionScheduler_Logger $logger, Justbee_PostCaster_ActionScheduler_QueueRunner $runner ) {
	 ...
	 * Prints the logs entries inline. We do so to avoid loading Javascript and other hacks to show it in a modal.
	 *
	 * @param Justbee_PostCaster_ActionScheduler_LogEntry $log_entry Log entry object.
	 * @param DateTimezone             $timezone Timestamp.
	 * @return string
	 */
	protected function get_log_entry_html( Justbee_PostCaster_ActionScheduler_LogEntry $log_entry, DateTimezone $timezone ) {
	...
	public function display_admin_notices() {
		...
		if ( ( is_a( $this->store, 'Justbee_PostCaster_ActionScheduler_HybridStore' ) || is_a( $this->store, 'Justbee_PostCaster_ActionScheduler_DBStore' ) ) && apply_filters( 'action_scheduler_enable_recreate_data_store', true ) ) {
			$table_list = array(
		...
		if ( $this->runner->has_maximum_concurrent_batches() ) {
			...
			$async_request_lock_expiration = Justbee_PostCaster_ActionScheduler::lock()->get_expiration( 'async-request-runner' );
	 ...
	 *
	 * @param Justbee_PostCaster_ActionScheduler_Schedule $schedule Action's schedule.
	 * @return string
	 */
	protected function get_schedule_display_string( Justbee_PostCaster_ActionScheduler_Schedule $schedule ) {
		...
		if ( is_a( $schedule, 'Justbee_PostCaster_ActionScheduler_NullSchedule' ) ) {
	...
	/**
	 * Implements the logic behind running an action. Justbee_PostCaster_ActionScheduler_Abstract_ListTable validates the request and their
	...
	protected function recreate_tables() {
		if ( is_a( $this->store, 'Justbee_PostCaster_ActionScheduler_HybridStore' ) ) {
			$store = $this->store;
		} else {
			$store = new Justbee_PostCaster_ActionScheduler_HybridStore();
		}
		add_action( 'action_scheduler/created_table', array( $store, 'set_autoincrement' ), 10, 2 );

		$store_schema  = new Justbee_PostCaster_ActionScheduler_StoreSchema();
		$logger_schema = new Justbee_PostCaster_ActionScheduler_LoggerSchema();
	...
	public function prepare_items() {
		...
		if ( 'past-due' === $this->get_request_status() ) {
			$query['status'] = Justbee_PostCaster_ActionScheduler_Store::STATUS_PENDING;
		...
		foreach ( $this->store->query_actions( $query ) as $action_id ) {
			...
			if ( is_a( $action, 'Justbee_PostCaster_ActionScheduler_NullAction' ) ) {
				continue;
			}
