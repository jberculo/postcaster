...
/**
 * Class Justbee_PostCaster_ActionScheduler_DataController
 *
...
class Justbee_PostCaster_ActionScheduler_DataController {
	...
	const DATASTORE_CLASS = 'Justbee_PostCaster_ActionScheduler_DBStore';

	/** Logger data store class name. */
	const LOGGER_CLASS = 'Justbee_PostCaster_ActionScheduler_DBLogger';
	 ...
	 * Instance.
	 *
	 * @var Justbee_PostCaster_ActionScheduler_DataController
	 */
	...
	public static function init() {
		if ( self::is_migration_complete() ) {
			add_filter( 'action_scheduler_store_class', array( 'Justbee_PostCaster_ActionScheduler_DataController', 'set_store_class' ), 100 );
			add_filter( 'action_scheduler_logger_class', array( 'Justbee_PostCaster_ActionScheduler_DataController', 'set_logger_class' ), 100 );
			add_action( 'deactivate_plugin', array( 'Justbee_PostCaster_ActionScheduler_DataController', 'mark_migration_incomplete' ) );
		} elseif ( self::dependencies_met() ) {
			Controller::init();
		}

		add_action( 'action_scheduler/progress_tick', array( 'Justbee_PostCaster_ActionScheduler_DataController', 'maybe_free_memory' ) );
	}
