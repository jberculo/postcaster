...
/**
 * Class Justbee_PostCaster_ActionScheduler_QueueRunner
 *
 * @license GPL-3.0-or-later
 * Modified by justbee on 08-May-2026 using {@see https://github.com/BrianHenryIE/strauss}.
 */
class Justbee_PostCaster_ActionScheduler_QueueRunner extends Justbee_PostCaster_ActionScheduler_Abstract_QueueRunner {
	const WP_CRON_HOOK = 'action_scheduler_run_queue';

	const WP_CRON_SCHEDULE = 'every_minute';

	/**
	 * Justbee_PostCaster_ActionScheduler_AsyncRequest_QueueRunner instance.
	 *
	 * @var Justbee_PostCaster_ActionScheduler_AsyncRequest_QueueRunner
	 */
	protected $async_request;

	/**
	 * Justbee_PostCaster_ActionScheduler_QueueRunner instance.
	 *
	 ...
	 * Get instance.
	 *
	 * @return Justbee_PostCaster_ActionScheduler_QueueRunner
	 * @codeCoverageIgnore
	 */
	public static function instance() {
		if ( empty( self::$runner ) ) {
			$class        = apply_filters( 'action_scheduler_queue_runner_class', 'Justbee_PostCaster_ActionScheduler_QueueRunner' );
	...
	/**
	 * Justbee_PostCaster_ActionScheduler_QueueRunner constructor.
	 *
	 ...
	 * @param Justbee_PostCaster_ActionScheduler_AsyncRequest_QueueRunner|null $async_request Async request runner object.
	 */
	public function __construct( ?Justbee_PostCaster_ActionScheduler_Store $store = null, ?Justbee_PostCaster_ActionScheduler_FatalErrorMonitor $monitor = null, ?Justbee_PostCaster_ActionScheduler_QueueCleaner $cleaner = null, ?Justbee_PostCaster_ActionScheduler_AsyncRequest_QueueRunner $async_request = null ) {
		parent::__construct( $store, $monitor, $cleaner );

		if ( is_null( $async_request ) ) {
			$async_request = new Justbee_PostCaster_ActionScheduler_AsyncRequest_QueueRunner( $this->store );
		}
	...
	public function maybe_dispatch_async_request() {
		...
		if (
			is_admin()
			&& ! Justbee_PostCaster_ActionScheduler::lock()->is_locked( 'async-request-runner' )
			&& Justbee_PostCaster_ActionScheduler::lock()->set( 'async-request-runner' )
		) {
	 ...
	 *
	 * @see Justbee_PostCaster_ActionScheduler_AsyncRequest_QueueRunner::handle()
	 *
	...
	public function run( $context = 'WP Cron' ) {
		Justbee_PostCaster_ActionScheduler_Compatibility::raise_memory_limit();
		Justbee_PostCaster_ActionScheduler_Compatibility::raise_time_limit( $this->get_time_limit() );
