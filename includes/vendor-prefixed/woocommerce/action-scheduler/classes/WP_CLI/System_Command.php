...
namespace Justbee\PostCaster\Vendor\Action_Scheduler\WP_CLI;

// phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaping output is not necessary in WP CLI.

use Justbee_PostCaster_ActionScheduler_SystemInformation as ActionScheduler_SystemInformation;
use WP_CLI;
...
class System_Command {
	...
	public function __construct() {
		$this->store = \Justbee_PostCaster_ActionScheduler::store();
	}
	...
	public function status( array $args, array $assoc_args ) {
		 ...
		 */
		$runner_enabled = has_action( 'action_scheduler_run_queue', array( \Justbee_PostCaster_ActionScheduler::runner(), 'run' ) );
	...
	public function version( array $args, array $assoc_args ) {
		...
		$instance = \Justbee_PostCaster_ActionScheduler_Versions::instance();
	 ...
	 * Get latest version.
	 *
	 * @param null|\Justbee_PostCaster_ActionScheduler_Versions $instance Versions.
	 * @return string
	 */
	...
	protected function get_current_runner() {
		return get_class( \Justbee_PostCaster_ActionScheduler::runner() );
	}
