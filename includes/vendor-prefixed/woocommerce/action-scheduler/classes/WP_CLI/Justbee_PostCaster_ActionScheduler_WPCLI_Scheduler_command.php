 ...
 */
class Justbee_PostCaster_ActionScheduler_WPCLI_Scheduler_command extends WP_CLI_Command {
	...
	public function fix_schema( $args, $assoc_args ) {
		$schema_classes = array( Justbee_PostCaster_ActionScheduler_LoggerSchema::class, Justbee_PostCaster_ActionScheduler_StoreSchema::class );

		foreach ( $schema_classes as $classname ) {
			if ( is_subclass_of( $classname, Justbee_PostCaster_ActionScheduler_Abstract_Schema::class ) ) {
				$obj = new $classname();
				$obj->init();
	...
	public function run( $args, $assoc_args ) {
		...
		Justbee_PostCaster_ActionScheduler_DataController::set_free_ticks( $free_on );
		Justbee_PostCaster_ActionScheduler_DataController::set_sleep_time( $sleep );

		$batches_completed = 0;
		$actions_completed = 0;
		$unlimited         = 0 === $batches;
		if ( is_callable( array( Justbee_PostCaster_ActionScheduler::store(), 'set_claim_filter' ) ) ) {
			$exclude_groups = $this->parse_comma_separated_string( $exclude_groups );

			if ( ! empty( $exclude_groups ) ) {
				Justbee_PostCaster_ActionScheduler::store()->set_claim_filter( 'exclude-groups', $exclude_groups );
			}
		}

		try {
			// Custom queue cleaner instance.
			$cleaner = new Justbee_PostCaster_ActionScheduler_QueueCleaner( null, $clean );

			// Get the queue runner instance.
			$runner = new Justbee_PostCaster_ActionScheduler_WPCLI_QueueRunner( null, null, $cleaner );
